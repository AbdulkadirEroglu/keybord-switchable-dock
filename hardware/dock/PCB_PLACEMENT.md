...
Parts that belong together on the dock PCB. Place each group as a cluster, then route between groups. References match `hardware/dock` (66 parts).
...
| **6** | **Host** (USB end → J1, **antenna end at the board edge**) | A1 | Raspberry Pi Pico 2 W | Socketed + 2 USB probes + 1×3 SWD socket | Keyboard host, router, BLE to the pad (GPIO map below) |
...
| **9** | **Pogo interface** (pad-docking edge) | J7 | 6-pin pogo connector | `dock:Pogo-6` | GND, +5V, DET, RX, TX, GND |
| | | U9 | TPD4E1U06DBVR | SOT-23-6 | ESD, **right at J7** |
| | | R25, R26, R27 | 1 kΩ | 0805 | Series resistors on TX/RX/DET, between U9 and the host (GP16/GP17/GP22) |
| | | R18 | 10 kΩ | 0805 | `POGO_DET` pull-up to `HOST_3V3` |
...
| R18 | `POGO_DET` | `HOST_3V3` (A1 pin 36) | pogo group or A1 right column |
...
| A1 host | PIO TX → pogo (`POGO_TX`) | GP16 | 21 | right, bottom |
| A1 host | PIO RX ← pogo (`POGO_RX`) | GP17 | 22 | right, bottom |
...
| A1 host | `POGO_DET` input | GP22 | 29 | right, middle |
| A1 host | SWD (SWCLK / GND / SWDIO) | debug holes | — | middle, 19.8 mm from the antenna end, via 1×3 socket |
...
| Female socket, 1×3, 2.54 mm | 1 | generic (cut from a longer strip) | Host SWD, on the dock |
| Male pin header, 1×20 / 1×3, 2.54 mm | 6 + 1 | generic | Soldered to the Picos (1×3 on the host's debug holes, pointing down) |
| Spring probe | 10 required, +5 optional | P50-B1 | A1 (Pico 2 W): TP2, TP3 (+TP1). A2/A3 (Pico 2): TP2, TP3, D1, D3 (+TP1, D2 GND) |
...
- Nothing under the modules except the probes. Probe tails stick out about 2.7 mm below the board at each HID Pico's two ends and at the host's USB end, so keep bottom-side parts away from those spots.
- Host (Pico 2 W) antenna end: at a board edge, with no copper under it and nothing in front of it. The footprint's 14 × 9 mm keep-out zone enforces the copper part. Use a non-metal enclosure.
- SWD headers J2, J3, J6: along one edge you can reach with the lid off.
...
- Board outline: 100 × 100 mm (60.5, 25.5) – (160.5, 125.5). The host sits at the bottom edge with its antenna end on the edge; the pogo group goes to a bottom corner, clear of the antenna.
...
- **`SYS_5V` trunk.** Run one wide horizontal track below the connector area and drop short branches to each Pico's pin 39 (top-right corner); one branch continues down the side to U8 and J7.
